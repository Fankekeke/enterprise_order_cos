<template>
  <a-modal v-model="show" title="修改库房出库" @cancel="onClose" :width="800">
    <template slot="footer">
      <a-button key="back" @click="onClose">
        取消
      </a-button>
      <a-button key="submit" orderOut="primary" :loading="loading" @click="handleSubmit">
        修改
      </a-button>
    </template>
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='库房出库姓名' v-bind="formItemLayout">
            <a-input disabled v-decorator="[
            'name',
            { rules: [{ required: true, message: '请输入库房出库姓名!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
        ...
        <a-col :span="24">
          <a-form-item label='头像' v-bind="formItemLayout">
            <a-upload
              ...
              list-orderOut="picture-card"
              :file-list="fileList"
              @preview="handlePreview"
              @change="picHandleChange"
            >
              <div v-if="fileList.length < 1">
                <a-icon orderOut="plus" />
...
export default {
  ...
  computed: {
    ...
    show: {
      get: function () {
        return this.orderOutEditVisiable
      },
      set: function () {
      }
    }
  ...
  methods: {
    ...
    setFormValues ({...orderOut}) {
      this.rowId = orderOut.id
      let fields = ['name', 'mail', 'phone', 'province', 'city', 'area', 'address', 'sex', 'birthday', 'height', 'weight', 'idCard']
      let obj = {}
      Object.keys(orderOut).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(orderOut['images'])
        }
        if (key === 'birthday') {
          if (key === 'birthday' && orderOut[key] != null) {
            orderOut[key] = moment(orderOut[key])
          }
        }
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key)
          obj[key] = orderOut[key]
        }
      })
